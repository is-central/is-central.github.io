...
## Tournament-style IS
IS tournaments are common enough that one is hosted every few months, allowing players worldwide to compete and show off their skills. Tournaments typically require playing at the highest difficulty and encourage clearing multiple endings in a single run for bonus points, creating drastically more restrictive gameplay conditions than those of casual play. Pathing is restricted by having to fulfil multiple ending conditions, and the negative effects of multiple ending-related collectibles are active at once. It's the most difficult that IS can get.

Many of the guides in our resource sections are written with these restrictive tournament conditions in mind, meaning the goal is to develop strategies that can clear multiple endings on the highest difficulty as consistently as possible. That doesn't mean these guides are useless for more causal players, though. Since the guides generally assume you're working in the most punishing conditions possible, the advice usually also works when you have more leeway. 
